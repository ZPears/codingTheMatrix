...
def noise(A,freq):
    ...
    The probability for one in any entry of the matrix is freq.
    """
    f = {(i,j):one for i in A.D[0] for j in A.D[1] if random.random() < freq}
    return mat.Mat(A.D, f)

def main():
    m = listlist2mat( [ [one,zero,one,one], [one,one,zero,one], [zero,zero,zero,one], 
        [one,one,one,zero], [zero,zero,one,zero], [zero,one,zero,zero], [one,zero,zero,zero] ] )
    result = matrix_vector_mul(m, [one,zero,zero,one])
