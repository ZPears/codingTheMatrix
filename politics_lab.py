...
def create_voting_dict(strlist):
    ...
    """
    return { x.split(' ')[0]:[ int(y) for y in x.split(' ')[3:] ] for x in strlist }
...
def policy_compare(sen_a, sen_b, voting_dict):
    ...
    """
    return sum([ voting_dict[sen_a][k] * voting_dict[sen_b][k] for k in range(len(voting_dict[sen_a])) ])
...
def most_similar(sen, voting_dict):
    ...
    for senator in voting_dict.keys():
        if senator != sen:
            diff = policy_compare(senator, sen, voting_dict)
            if diff > highest:
                highest = diff
                most_sim = senator
    return most_sim
...
def find_average_record(sen_set, voting_dict):
    ...
    Example: 
        ...
        [0.0, 1.0, 1.0]
    """
    new_dict = {k:voting_dict[k] for k in sen_set if k in voting_dict}
    results = []
    num_votes = len(list(new_dict.values())[0])
    num_sens = len(new_dict.items())
    for i in range(num_votes):
        votes = [ list(new_dict.values())[j][i] for j in range(num_sens) ]
        results.append(sum(votes)/len(votes))
    #[ sum(i)/len(i) for i in [ list(new_dict.items())[i][1][k] for k in range(len(list(new_dict.items())[0][1])) for i in range(len(new_dict.items())) ]
    return results
...
def bitter_rivals(voting_dict):
    ...
    biggest_diff = 50
    rivals = ()
    for senator in list(voting_dict.keys()):
        nemesis = least_similar(senator, voting_dict)
        diff = policy_compare(senator, nemesis, voting_dict)
        if diff < biggest_diff:
            biggest_diff = diff
            rivals = (senator, nemesis)
    return rivals
