...
def equal(u,v):
    ...
    for k in v.D:
        if v[k] != u[k]:
            result = False
            break
    #pairsV = [ [x,y] for x,y in v.f.items() if y != 0 ]
    #pairsU = [ [x,y] for x,y in u.f.items() if y != 0 ]
    #for i in range(len(pairsV)):
    #    if pairsV[i] != pairsU[i]:
    #        result = False
    #        break
    return result
...
def add(u,v):
    ...
    newF = {}
    for k in u.D:
        # if (u[k] is not None) & (v.f.get(k) is not None):
        newF[k] = u[k] + v[k]
        #if newF[k] == 0:
        #    del newF[k]
...
def neg(v):
    ...
    >>> u == Vec(set([2,4,6,8]),{2:1,4:2,6:3,8:4})
    True
