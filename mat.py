...
def vector_matrix_mul(v, M):
    ...
    """
    assert M.D[0] == v.D
    return Vec(M.D[1], { col : sum([ M[(row,col)] * v[row] for row in v.D ]) for col in M.D[1] })
...
def matrix_vector_mul(M, v):
    ...
    """
    assert M.D[1] == v.D
    return Vec(M.D[0], { row : v*Vec(M.D[1], { j:v for (i,j),v in M.f.items() if i == row }) for row in M.D[0] } )
...
def matrix_matrix_mul(A, B):
    ...
    for row in A.D[0]:
        for col in B.D[1]:
            r = Vec(A.D[1], { j:v for (i,j),v in A.f.items() if i == row })
            c = Vec(B.D[0], { i:v for (i,j),v in B.f.items() if j == col })
            result = r*c
            if result != 0:
                M[row,col] = result
    return M
